...
export const AdminTelegramContestsSection: React.FC<AdminTelegramContestsSectionProps> = ({ currentUser, showToast }) => {
    ...
    const generateDescriptionWithAI = useCallback(async () => {
        ...
        try {
            const prompt =
                'Ты — креативный копирайтер, специализирующийся на увлекательных анонсах для Telegram.\n' +
    ...
    const handleSubmitContest = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // console.log("Payload to Edge Function (AdminContestsSection):", payload); // Removed for cleaner logs in production
            ...
            if (functionError) {
                ...
                // console.warn("Function response did not explicitly state success or error.", data); // Removed for cleaner logs
